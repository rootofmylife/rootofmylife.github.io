...
MCP allows tools to be added at runtime instead of design time.

And _because_ this can happen at runtime, the user (NOT the developer) can add arbitrary functionality to the LLM application (while the application is running — hence, runtime). One could make the argument that LLM applications with MCP support are conceptually similar to browsers — both let users connect to arbitrary MCP/HTTP servers at runtime.

## Compare to SOAP and WSDL

SOAP is a protocol for exchanging XML-based messages over a network. WSDL is an XML-based language for describing web services and how to access them.

WSDL is a design-time protocol. It describes the web service and how to access it. MCP is a runtime protocol. It allows tools to be added at runtime instead of design time.
...
- [Reddit](https://www.reddit.com/r/ClaudeAI/comments/1h3g01r/want_to_ask_claude_about_model_context_protocol/)

## Ref

- [ondr](https://www.ondr.sh/blog/thoughts-on-mcp)
